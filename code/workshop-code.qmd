...
# create new objwxr for clean data
drought_exp_clean <- drought_exp |> 
  # remove spaces, add lowercases and replaces spaces with underscores
  clean_names() |> 
  # creating new column with full scientific name
  # change species names abbreviated into full name
  ...
  # create new column for full treatment name
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # store species name as factor, reorder data
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # select columns of interest from clean data
...
slice_sample(drought_exp_clean, #clean data frame to display
             n = 10) # show 10 rows
```
...
```{r}
#base layer ggplot, x = sla, y - total plant mass
ggplot(drought_exp_clean, aes(x = sla, y = total_g)) +
  #scatterplot
  geom_point() +
  #labs relabeling axis, adding title
  labs(title = "as SLA increases, total mass increases", 
       x = expression(
         "Specific leaf area (cm"^2~"/g)"), y = "Total plant mass (g)")
#can also use cm\U00B2/ g)
...
#| fig-height: 6
#| fig-width: 8
ggplot(drought_exp_clean, aes(x = water_treatment, 
                              y = total_g, color = water_treatment)) +
  geom_jitter(height = 0, width = 0.1) +
  scale_color_manual(values = c(
    "Drought stressed" = ds_col, 
    "Well watered" = ww_col)) +
  labs(title = "Well watered individuals tend to have higher mass than 
       drought stress", 
       x = "Treatment", y = "Total plant mass (g)")+
...
ggplot(drought_exp_clean, aes(x = species_name, y = total_g)) +
...
- no outliers that could be influencing model coefficeinet estimates
(leverage)
...
-`(Intercept)`: The intercept is the model prediction for plant mass 
for Lotus scoparius in the drought stressed treatment. 
-`water_treatmentWellWatered`: model prediction for the difference
in Lotus scoparius mass between well watered and drought stressed treatments
-`species_namePenstemon centrathifolious`:  difference between 
Penstemon mass in well watered treatment and Lotus scoparous mass in the 
well watered treatment. 
- `species_nameSalvia leucophylla`: difference between Eschscholzia californica 
mass in the well watered treatment and Lotus scoparois mass in the well 
watered treatment. 
...
preds <- ggpredict(
  ...
  #renaming columns to fit orignial data frame
...
#| fig-width: 6
#base layer ggplot
ggplot(drought_exp_clean, #data set
       aes(x = water_treatment, #x = water treatment
           y = total_g)) + # y = total mass
  geom_jitter(height = 0, width = 0.1) + #jitterplot
  
  geom_pointrange(preds,mapping = aes( #adds confidence level in point range
    x = water_treatment, 
    y = predicted, 
    ymin = conf.low, 
    ymax = conf.high)) +
